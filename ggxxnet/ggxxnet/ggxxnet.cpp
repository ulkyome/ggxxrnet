...
void ggn_softReset(void)
{
	...
	// Issue ‚PDIt is not too late and data reception of spectators has not kept pace
	// Issue ‚QDexe Not sent until the last when dropped itself (although this is not the way ...)
	Sleep(300);
...
void ggn_endBattle(void)
{
	...
	else if (*GGXX_MODE1 & 0x800000) // ЉПђнЏI—№
	{
		// Add a termination to replay
		g_netMgr->disconnect("endbattle");
	}
...
/* Drawing */
	GGXX_DrawText2("REPLAY", 40, 53, 5); //текст в реплее оглавление
