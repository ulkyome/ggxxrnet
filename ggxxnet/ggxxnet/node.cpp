...
int CNode::getPingAv(void)
{
	...
	/* In one Toka it lack credibility. Or saying if it is too increased and the lead will not be */
...
int CNodeMgr::addNode(char* p_addr, char* p_name, bool p_busy, bool p_needUHP)
{
	...
	// Also ignored if m_ownNode is not acquired yourself to ignore
	if (useLobbyServer())
	{
	...
	int idx = -1;
	// UNKNOWN_NAME be disagreement unconditionally
	...
	if (idx == -1)
	{
		...
		/* Ask the diffusion of port periodically to the other node */
		node.m_uhpNodeIdx	= 0;
	...
	else
	{
		if (findNodeIdx_address(p_addr) == -1)
		{
			...
			// Do not return a idx because // not a new addition; return idx
...
int CNodeMgr::findNodeIdx_address(char* p_addr)
{
	...
	{
		// If it matches with any of tmpaddr from 1.15-3 and was ok
...
/* comparison function to be used in the sortNodeList */
