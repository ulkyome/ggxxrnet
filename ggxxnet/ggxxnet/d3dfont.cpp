...
int CD3DFont::getTextWidth(wchar_t* p_str, int p_line)
{
	...
	for (int i = 0; i < len; i++)
	{
		if (p_str[i] == L'\n')
		{
			w = 0;
			line++;
			if (p_line != -1 && line > p_line) break;	/* It finished the purpose of the line */
			continue;
		}
		if (line != p_line && p_line != -1) continue;	/* Unrelated lines are not read */
...
CFontObj* CD3DFont::createfobj(wchar_t p_ch)
{
	...
	if (m_antiAlias == 0)
	{
		...
		if (m_outline > 0)	/* Disintermediation */
		{
			for (UINT y = 0; y < gm.gmBlackBoxY; y++)
			{
			...
			for (UINT x = 0; x < gm.gmBlackBoxX; x++)
			{
				...
				for (int i = 0; i <= m_outline << 1; i++)
				{
					...
					for (int j = 0; j <= m_outline << 1; j++)
					{
						...
						{
							// The first place because 1bit only - channel is not AA is non-representation
	...
	/* Delete what is original when Register fobj it is already */
	if (m_fobj[m_fobjpos])
	{
