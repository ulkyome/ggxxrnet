...
BYTE*  const GGXX_1PCHEAT	= (BYTE*)0x5f958c; /* 0 = normal, 1 = gold, 2 = black */
BYTE*  const GGXX_2PCHEAT	= (BYTE*)0x5f9590; /* 0 = normal, 1 = gold, 2 = black */
...
/* Deviate by random selection When this suit not */
...
/* Character selector cursor storage */
...
//******************************************************************
/* Variables, and functions that are referenced from GGXX to ensure the memory space on the GGXX */
...
// GGXX functions
//*******************************************************************
/* Call the function of GGXX from ggxxnet */
...
void GGXX_GetInputFromDI(void)
{
	...
	// Integration of analog input here
	for (int i = 0; i < 2; i++)
	{
...
void GGXX_SetAction(DWORD p_data, int p_actid)
{
	__asm
	{
		...
		add esp, 8
		mov eax, p_data
//		mov word ptr [eax + 0xe0], 10	//I want to also change frame
	}
}
