...
DWORD getSysDiskSN(void)
{
	...
	if (GetDriveType(drivelabel) == DRIVE_FIXED)
	{
		GetVolumeInformation(
			drivelabel,	// Root directory
			NULL,		// Volume name buffer
			0,			// The size of the name buffer
			&sn,		// Serial number of the volume
			0,			// The maximum length of the file name
			0,			// File System Options
			NULL,		// Buffer to store the file system name
			0);			 // The size of the buffer that stores the file system name
		//char str[9];
		//sprintf(str, "%08x", sn);
